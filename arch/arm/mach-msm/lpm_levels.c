...
static uint32_t sysfs_dbg_l2_mode __initdata = MSM_SPM_L2_MODE_POWER_COLLAPSE;
...
int touch_i2c_param_init;
static int lpm_enter_change_idle_state(int idx) {
	int new_idx = idx;

	if (touch_i2c_param_init == 1)
		idx = 0;
		...
		struct cpuidle_driver *drv, int index)
{
	...
	idx = lpm_enter_change_idle_state(idx);
